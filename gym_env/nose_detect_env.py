...
# define how state move
    def step(self, action):
        ...
        elif (action==[0,1,0,0,0]).all(): # right
            self.x = min(self.x+1, self.nrow-self.width-2)
            new_state = self.desc[self.x:self.x+self.width,self.y:self.y+self.height]
            new_state_pt = [self.x+13.5, self.y+13.5]
            new_state_pt = np.asarray(new_state_pt)
        elif (action==[0,0,1,0,0]).all(): # down
            self.y = min(self.y+1, self.ncol-self.height-2)
            new_state = self.desc[self.x:self.x+self.width,self.y:self.y+self.height]
            new_state_pt = [self.x+13.5, self.y+13.5]
            new_state_pt = np.asarray(new_state_pt)
        elif (action==[0,0,0,1,0]).all(): # left
        ...
        if  (action == [1,0,0,0,0]).all() and self._get_IoU(self.gt_box, new_state_pt_box)>0.7:
            done = 1
            reward = 1
...
#            reward = 1
        ...
        elif (action == [1,0,0,0,0]).all() and self._get_IoU(self.gt_box, new_state_pt_box)<0.7:
            done = 1
            reward = -1
